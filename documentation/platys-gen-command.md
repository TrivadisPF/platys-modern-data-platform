...
```
Usage: platys.py gen [OPTIONS]

  Generates all the needed artefacts for the docker-based modern (data)
  platform.
...
Options:
  ...
  --structure [flat|subfolder]    defines the where the stack will be
                                  generated flat : as in same folder as script
                                  generate the stack into same folder as
                                  config.ymlsubfolder : generate the stack
                                  into a subfolder, which by default is the
                                  name of the platform provided when
