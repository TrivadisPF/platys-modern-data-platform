...
@click.option('--del-empty-lines/--no-del-emptylines', 'del_empty_lines', default=True, show_default=True,
              help='remove empty lines from the docker-compose.yml file.')
@click.option('--flat', 'stackorg', flag_value="flat", default=True,
              help='generate the stack into same folder as stack-config.yml')
@click.option('--subfolder', 'stackorg', flag_value="subfolder",
              help='generate the stack into a subfolder, which by default is the name of the platform provided when initializing the stack')
...
#
@cli.command()  # @cli, not @click!
@click.option('-n', '--platform-name', 'platform_name', type=click.STRING, required=True,
              help='the name of the platform stack to generate.')
@click.option('-in', '--stack-image-name', 'stack_image', default='modern-data-platform-stack-generator',
              type=click.STRING, show_default=True, help='the modern data platform stack image')
@click.option('-iv', '--stack-image-version', 'stack_version', default='LATEST', type=click.STRING, show_default=True,
              help='the modern data platform stack image version to use')
@click.option('-cf', '--config-filename', 'config_filename', default='stack-config.yml', type=click.STRING,
              show_default=True, help='the name of the local config file.')
@click.option('-ps', '--predef-stack-config', 'predef_stack_config', type=click.STRING,
              help='the name of a predefined stack to base this new platform on')
@click.option('-f', '--force', is_flag=True, help='If specified, this command will overwrite any existing config file')
def init(platform_name, stackimage_name, stackimage_version, config_filename, predef_stack_config, force):
